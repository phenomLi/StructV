...
class LinkModel {
    ...
    addLinkPair(linkInfo) {
        if (linkInfo.target === null)
            return;
        let element = linkInfo.element, target = linkInfo.target, id = `${element.elementId}-${target.elementId}`, linkName = linkInfo.linkName, label = linkInfo.label || this.linkOptions[linkName].label, sourceTarget = linkInfo.sourceTarget, anchorPair = linkInfo.anchorPair, index = linkInfo.index, linkOption = this.linkOptions[linkName], linkShape = this.viewModel.createShape(id, 'line', linkOption);
    ...
    applyCanceledLink(elementList) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (this.linkPairs.find(item => item.id === lastLinkPair.id) === undefined) {
                let element = elementList.find(item => item.elementId === lastLinkPair.ele.elementId), target = elementList.find(item => item.elementId === lastLinkPair.target.elementId);
                if (element) {
