...
export class LinkHandler {
     ...
     * @param elementList 
     */
    bindLinkShape(linkOptions: { [key: string]: LinkOption}, elementList: Element[]) {
                ...
                if(Array.isArray(ele[linkName])) {
                    ele[linkName].map((item, index) => {
                        ...
                        this.linkPairs.push({
                            ...
                            ele: ele,
                            target: item,
                            anchorPair: contact? [ele.anchors[contact[0]], item.anchors[contact[1]]]: contact,
                            anchorPosPair: null,
                    ...
                    this.linkPairs.push({
                        ...
                        ele: ele,
                        target: ele[linkName],
                        anchorPair: contact? [ele.anchors[contact[0]], ele[linkName].anchors[contact[1]]]: contact,
                        anchorPosPair: null,
                        label: this.labelSolver(linkOption.label, ele, ele[linkName]),
                        index: null,
                        dynamic: contact? false: true
                    });
                }
    ...
    private linkElement(linkPair: LinkPair) {
        ...
        linkShape = this.dataModel.createLineShape(`${element.elementId}-${target.elementId}`, linkOption);
     ...
     *      \---*---\
     *          2
     * - 第二个参数为自定义的锚点，格式为 { number：anchor }，若number为0或1或2或3或4，则覆盖之前的锚点
     * @param ele
     */
    public getAnchors(ele: Element): anchor[] {
        let anchors = this.engine.layoutOption[ele.name].anchors,
            hw = ele.shape.width / 2, hh = ele.shape.height / 2,
            defaultAnchors: [number, number][] = [
                [0, -hh], [hw, 0], [0, hh], [-hw, 0], [0, 0]
            ];

        if(anchors) {
            Object.keys(anchors).map(key => {
                defaultAnchors[key] = anchors[key];
            });
        }
